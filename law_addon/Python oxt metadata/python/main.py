...
def MessageBox(ParentWin,
    ...
    myBox = sv.createMessageBox(
        ParentWin, MsgType, MsgButtons, MsgTitle, MsgText)
...
def update_indices(*args):
    """
    Update content tables iterating through each one given by getDocumentIndexes()
    """
    Doc = XSCRIPTCONTEXT.getDocument()
...
def insert_law(*args):
    """
    Insert law body usind data given by gsoc2018-3gm project database
    """
    ...
    try:
        ui_locale = gettext.translation('base',
                                        localedir=get_main_directory("com.addon.lawaddon") +
                                        'python/locales',
    ...
    dlg = dp.createDialog(
        "vnd.sun.star.extension://com.addon.lawaddon/dialogs/InsertLaw.xdl")
    ...
    HelpLabel.Label = _(
        "Use format [number]/[year] for searching in law database")
    ...
    if not re.search('\d+\s?\/\s?\d+', LawIDString):
        ...
        MessageBox(parentwin,
                   _("Law ID field not properly filled, please try again"),
                   _('Law ID Error'),
                   ERRORBOX)
        insert_law()
        return
    ...
    response = requests.get(
        "https://3gm.ellak.gr/get_law/" + LawType + "/" + LawIDString)

    if response.status_code == 404:
        parentwin = Doc.getCurrentController().Frame.ContainerWindow
        MessageBox(parentwin, _(
            "Failure while communicating with server, please try later"),
            _('Error'),
            ERRORBOX)
        return
    ...
    try:
        ...
        # Pythonic get the last inserted element in list
        Articles = Versions[-1]['articles']
    except Exception as e:
        parentwin = Doc.getCurrentController().Frame.ContainerWindow
        MessageBox(parentwin, _(
            "Failure while communicating with server, please try later"),
    ...
    if re.search('\d+\s?\-\s?\d+', ArticleField.Text):
        ...
        parentwin = Doc.getCurrentController().Frame.ContainerWindow
        MessageBox(parentwin,
                   _("Empty or wrong Articles input, please try again"),
                   _('Law ID Error'),
                   ERRORBOX)
        insert_law()
        return
...
def print_paragraphs(article_body, ViewCursor):
    """
    Print all paragraphs from 3gm response api
    """
    for paragraph_num, paragraph_body in sorted(article_body.items(),
...
def insert_contents(*args):
    """
    Fire up isnert content table dialog using dispatch commands
    """
    ctx = uno.getComponentContext()
    smgr = ctx.ServiceManager
...
def insert_external_document(*args):
    """
    Insert external document using file link as a result of file dialog
    """
    doc = XSCRIPTCONTEXT.getDocument()
